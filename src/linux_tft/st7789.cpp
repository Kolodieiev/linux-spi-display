...
{
  beginWrite();
  writeCommand((state) ? ST7789_INVON : ST7789_INVOFF);
  endWrite();
}

void write_window_addr(int16_t x, int16_t y, uint16_t w, uint16_t h)
{
  writeC8D16D16(ST7789_CASET, x, w - 1);

  writeC8D16D16(ST7789_RASET, y, h - 1);
...
void init_display()
{
  ...
  batch_operations(st7789_init_operations, sizeof(st7789_init_operations));
}
